"""
test_custom_query_2.py — Custom Query 2: Vanishing Gradient (Semantic Recall)

Query:
  "How do deep networks avoid the problem of learning signals becoming too
   small to be useful during training?"

Type: SEMANTIC RECALL — the query words ("learning signals", "too small",
"useful") do NOT appear literally in the matching corpus chunks. The vector
embedding bridges the gap to "vanishing gradient", "residual connections",
"batch normalization", "ReLU", etc.

Tests (both run in one go)
───────────────────────────
  test_custom_query_2_with_index
      POST /rag with the full corpus available.
      Asserts: confidence="high", max_score ≥ 0.70, answer contains vanishing-
      gradient mitigation keywords, sources reference the Vanishing Gradient /
      Batch Normalization corpus pages, and query literal words are absent from
      the top retrieved chunk (FR-7.3).

  test_custom_query_2_without_index
...
Run (requires rag_server :8108 + gateway :8107):
    uv run pytest tests/test_custom_query_2.py -v
    uv run pytest tests/test_custom_query_2.py -v -s     # show print output
    uv run python  tests/test_custom_query_2.py          # standalone script
"""
...
TRACE_WITH    = ROOT / "traces" / "custom" / "query_2_with_index.json"
TRACE_WITHOUT = ROOT / "traces" / "custom" / "query_2_without_index.json"
...
# At least one must appear in the answer — vanishing-gradient mitigations.
GRADIENT_SIGNALS = [
    "vanishing gradient",
    "residual connection",
    "residual network",
...
# At least one source must come from the vanishing-gradient / batch-norm /
...
# to prove semantic (not keyword) retrieval (FR-7.3).
QUERY_LITERAL_WORDS = ["learning signals", "too small", "useful"]
...
def test_custom_query_2_with_index() -> None:
    """
    ...
    Checks (all must pass):
      ...
      • answer contains a vanishing-gradient mitigation keyword
      • at least one source references Vanishing Gradient / Batch Norm corpus doc
      • top retrieved chunk does NOT contain the literal query words
        (proves semantic, not keyword, retrieval — FR-7.3)
      • traces/custom/query_2_with_index.json is consistent
    """
    print(f"\n{'─'*60}")
    print("test_custom_query_2_with_index")
    print(f"{'─'*60}")
    ...
    # ── answer contains gradient-mitigation concept ───────────────────────────
    answer_low     = answer.lower()
    matched_signal = next((s for s in GRADIENT_SIGNALS if s in answer_low), None)
    assert matched_signal is not None, (
        f"answer does not mention any vanishing-gradient mitigation.\n"
        f"Signals checked: {GRADIENT_SIGNALS}\n"
        f"Answer: {answer[:300]}"
    )
    _ok("answer contains gradient-mitigation concept", f"'{matched_signal}'")
    ...
    assert matched_src is not None, (
        f"no source references a gradient/batch-norm corpus document.\n"
        f"Signals checked: {SOURCE_SIGNALS}\n"
        f"Sources returned: {[s.get('source', '') for s in sources]}"
    )
    _ok("sources reference gradient/batch-norm corpus doc", f"signal='{matched_src}'")
...
def test_custom_query_2_without_index() -> None:
    """
    ...
    Checks (all must pass):
      ...
      • sources == []
      • traces/custom/query_2_without_index.json is consistent
    """
    print(f"\n{'─'*60}")
    print("test_custom_query_2_without_index")
    print(f"{'─'*60}")
...
if __name__ == "__main__":
    ...
    print("=" * 78)
    print("CUSTOM QUERY 2 — Vanishing Gradient  (Semantic Recall, FR-7.3)")
    print(f"Query: {QUERY}")
    print("=" * 78)
    ...
    for fn, label in [
        (test_custom_query_2_with_index,    "with_index"),
        (test_custom_query_2_without_index, "without_index"),
    ]:
        try:
            fn()
            results[label] = "PASS ✓"
